...
describe("LandingPage", () => {
  it("presents CoinBurrow as a real-time crypto dashboard", () => {
    ...
    expect(text).toContain("Bury");
    expect(text).toContain("Made by @HappyMarmot123");
    const creatorLink = wrapper.find(".creator-link");
    expect(creatorLink.exists()).toBe(true);
    expect(creatorLink.attributes("href")).toBe("https://github.com/HappyMarmot123");
    expect(creatorLink.attributes("target")).toBe("_blank");
    expect(creatorLink.attributes("rel")).toBe("noopener noreferrer");
    ...
    expect(source).toContain("function setupCursorEffects()");
    expect(source).toContain("@keyframes cursor-spark-pop");
    expect(source).toContain("@media (pointer: coarse), (prefers-reduced-motion: reduce)");
    expect(source).not.toContain("eyebrow-motion");
    expect(source).not.toContain("translate: 0 -6px;");
