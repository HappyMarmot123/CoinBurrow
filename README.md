...
- KRW 마켓 기준 코인 목록과 주요 시세 조회
- Upbit REST 기반 초기 스냅샷 로딩
- Upbit WebSocket 기반 실시간 ticker, candle, orderbook, trade 스트림
- Web Worker를 이용한 실시간 데이터 처리 분리
- RxJS 기반 버퍼링, 스로틀링, 채널별 스트림 처리
- Pinia 기반 화면 상태 관리
- Highcharts / lightweight-charts 기반 차트 렌더링
- 글로벌 시총, 시장심리, 김치프리미엄을 모은 인사이트 페이지
- Vercel 정적 SPA + 서버리스 API 배포 구조
...
| [#5 기능 / 페이지 명세서](https://github.com/HappyMarmot123/CoinBurrow/issues/5)            | 초기 제품 아이디어와 기능 범위 정리                                                     |
